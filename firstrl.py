...
def player_death(player):
    #the game ended!
    global game_state
    message('You died!', color = libtcod.red)
    game_state = 'dead'
...
def monster_death(monster):
    ...
    message(monster.name.capitalize() + ' is dead! You gain ' + str(monster.fighter.xp) + ' experience points.', color = libtcod.orange)
    monster.char = '%'
...
def render_all():
    ...
    if fov_recompute:
        #recompute FOV if needed (the player moved or something changed)
        fov_recompute = False
...
def handle_keys():
    ...
    if game_state == 'playing':
        ...
        if key.vk == libtcod.KEY_UP or key.vk == libtcod.KEY_KP8:
            ...
            pass  #do nothing, i.e. wait for the monster to come to you
        else:
            #test for other keys
            key_char = chr(key.c)
